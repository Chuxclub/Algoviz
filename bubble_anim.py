...
def animate(i, ax, palette, colors, counter, counter2, x, recipe):
    ...
    for l in recipe[i]:
        ...
        plt.xticks(np.arange(1, len(x) + 1, 1), x_ticks_labels) # xticks(x_ticks_pos, x_labels)

    # Setting xticks colors (erased by cla() so current counter works):
    ax.get_xticklabels()[counter[0]].set_color(palette["highlighted"])
    ax.get_xticklabels()[counter[0]].set_fontsize(16)

    # Adding a vertical line which symbolises what has been sorted and what's yet to be sorted
    plt.axvline(x=15.5 - counter2[0], color=palette["highlighted"])

    # Plotting
    plt.bar(x, recipe[i], color=colors)

    # Preparing next bar colors (global variable, first call of animate => preset):
    colors[counter[0]] = palette["others"]
    colors[counter[0]+1] = palette["highlighted"]
    counter[0] += 1

    if counter[0] >= (len(x) - 1 - counter2[0]):
        colors[counter[0]] = palette["others"]
...
def bubble_anim():
    ...
    ani = FuncAnimation(plt.gcf(), animate, frames=len(recipe), fargs=(ax, palette, colors, counter, counter2, x, recipe,), repeat_delay=2000, interval=100)
